"""
Logging module for tracking LLM usage, token consumption, and application flow
"""
import json
import logging
...
class PropheticLogger:
    """
    Centralized logger for tracking:
    - LLM API calls (input/output, tokens, costs)
    ...
    def __init__(self, log_dir: str = "logs", session_name: Optional[str] = None):
        """
        Initialize logger with structured logging to files

        Args:
            log_dir: Directory to store log files
            session_name: Optional explicit session name for grouping logs
        """
        self.log_dir = Path(log_dir)
        self.log_dir.mkdir(exist_ok=True)

        # Generate rich session name if not provided
        import uuid, os as _os
        auto_name = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}-{_os.getpid()}-{uuid.uuid4().hex[:8]}"
        self.session_name = session_name or auto_name

        # Log file paths
        self.general_log_file = self.log_dir / f"app_{datetime.now().strftime('%Y%m%d')}.log"
        self.llm_log_file = self.log_dir / f"llm_{datetime.now().strftime('%Y%m%d')}.jsonl"
        self.session_log_file = self.log_dir / f"session_{self.session_name}.json"
        ...
        if not any(isinstance(h, logging.FileHandler) and getattr(h, 'baseFilename', '') == str(self.general_log_file) for h in self.logger.handlers):
        ...
        self.session_data["llm_calls"].append(call_data)
        
        # Write to JSONL file (one JSON object per line)
        with open(self.llm_log_file, 'a', encoding='utf-8') as f:
            f.write(json.dumps(call_data, ensure_ascii=False) + '\n')
        
        # Log summary to general log
        tokens_str = f"[{input_tokens or '?'} in / {output_tokens or '?'} out]" if (input_tokens or output_tokens) else ""
        self.logger.info(f"LLM Call: {model} {tokens_str} - {metadata.get('purpose', 'general')}")
        
        # Log prompt/response at debug level (can be verbose)
        self.logger.debug(f"Prompt: {prompt[:200]}...")
        self.logger.debug(f"Response: {response[:200]}...")
    
    def log_event(self, event_type: str, event_name: str, details: Optional[Dict[str, Any]] = None):
        """
        Log an application event
        
        Args:
            event_type: Type of event (calendar_loaded, alert_triggered, etc.)
    ...
    def print_summary(self):
        ...
        self.logger.info("="*60)
        self.logger.info("SESSION SUMMARY")
        self.logger.info(f"Start: {summary['session_start']}")
        self.logger.info(f"LLM Calls: {summary['llm_calls_count']}")
        self.logger.info(f"Total Input Tokens: {summary['total_tokens']['input']}")
        self.logger.info(f"Total Output Tokens: {summary['total_tokens']['output']}")
        self.logger.info(f"Total Tokens: {summary['total_tokens']['total']}")
        self.logger.info(f"Events Logged: {summary['events_count']}")
        self.logger.info("="*60)
...
def get_logger() -> PropheticLogger:
    """Get or create global logger instance"""
    global _logger_instance
    if _logger_instance is None:
        _logger_instance = PropheticLogger()
    return _logger_instance
